...
const issueResult = fs.readFileSync(
    './__tests__/mock/list-repo-issue.json',
    'utf8'
)
    ...
    (comment: IComment, content?: ReactionContent): Promise<Reaction[]> => {
        if (comment.id === 986013801) {
            return delay(
                JSON.parse(
                    fs.readFileSync(
                        './__tests__/mock/issue-comment-986013801-reactions.json',
                        'utf8'
                    )
                )
            )
        ...
        if (comment.id === 986020479) {
            return delay(
                JSON.parse(
                    fs.readFileSync(
                        './__tests__/mock/issue-comment-986020479-reactions.json',
                        'utf8'
                    )
                )
            )
...
const comments = fs.readFileSync(
    './__tests__/mock/issue-1-comments.json',
    'utf8'
)
